setTimeout(() => {
  const rule = window.evolv.renderRule.test;
  ...
  rule.whenDOM('.evolv-body').then((el1) => {
    const body = el1.firstDom();
    body.classList.add('whenDOM_tagged');
    body.insertAdjacentHTML('afterbegin', '<p>whenDOM was here</p>');

    rule.whenDOM('.evolv-learnButton').then((el2) => {
      const learnButton = el2.firstDom();
      learnButton.textContent = 'Learn things';
    });
  });

  // Confirm "whenItem" works
  rule.whenItem('bottomContainerSection').then((el1) => {
    const bottomContainerHeadline = el1.firstDom();
    bottomContainerHeadline.classList.add('whenItem_tagged');

    rule.whenItem('bottomContainerSectionH2').then((el2) => {
      const headlineEl = el2.firstDom();
      headlineEl.classList.add('whenItem_tagged');
    });
  });
  rule.whenItem('bottomContainerSectionH2_first').then((el) => {
    const headlineEl = el.firstDom();
    headlineEl.textContent = 'Headline 1';
  });
  rule.whenItem('bottomContainerSectionH2_second').then((el) => {
    const headlineEl = el.firstDom();
    headlineEl.textContent = 'Headline 2';
  });
  rule.whenItem('bottomContainerSectionH2_third').then((el) => {
    const headlineEl = el.firstDom();
  ...
  document.querySelector('.text-output').textContent = $('.jumbotron p').text();
  ...
  $('.number-gallery-item').firstDom().style.backgroundColor = 'rgb(183 63 63)';
  $('.number-gallery-item').lastDom().style.backgroundColor = 'rgb(183 63 63)';
