setTimeout(() => {
  ...
  rule.whenMutate().then(() => {
    $('.when-methods .row')
      .markOnce('when-mutate')
      .append(
        $(`<div class="when-mutate col-md-4">
                        <h3>whenMutate</h3>
  ...
  rule.whenElements('.when-methods.delayed-class .row > div').then((div) => {
    ...
    $('.when-methods .row')
      .markOnce('when-elements')
      .append(
        $(`<div class="when-elements col-md-4">
                <h3>whenElements</h3>
  ...
  function selectTab() {
    const tabIndex = parseInt(this.dataset.tabIndex, 10);
